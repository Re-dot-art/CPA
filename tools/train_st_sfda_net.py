...
# ================== Pseduo-labeling ==================
# =====================================================
def threshold_bbox(proposal_bbox_inst, new_class_thresholds, proposal_type="roih"):
    if proposal_type == "rpn":
        valid_map = proposal_bbox_inst.objectness_logits > 0.9
        ...
        valid_map = torch.zeros_like(proposal_bbox_inst.scores, dtype=torch.bool)
        
        for class_idx in range(len(new_class_thresholds)):
            class_valid_map = (proposal_bbox_inst.pred_classes == class_idx) & (proposal_bbox_inst.scores > 0.7)
            valid_map = valid_map | class_valid_map
...
def process_pseudo_label(proposals_rpn_k, new_class_thresholds, proposal_type, psedo_label_method=""):
    list_instances = []
    num_proposal_output = 0.0
    for proposal_bbox_inst in proposals_rpn_k:
        if psedo_label_method == "thresholding":
            proposal_bbox_inst = threshold_bbox(
                proposal_bbox_inst, new_class_thresholds, proposal_type=proposal_type
            )
        else:
...
# useless
def update_class_scores(scores, pred_classes, dynamic_thres_class, thres):
    device = scores.device 
    for i in range(len(pred_classes)):
        class_idx = pred_classes[i].item()
        if class_idx not in dynamic_thres_class:
            dynamic_thres_class[class_idx] = 0
        if scores[i] > thres:
            dynamic_thres_class[class_idx] += 1
    max_count = max(dynamic_thres_class.values())
    new_class_thresholds = torch.tensor([dynamic_thres_class.get(i, 0) / max_count for i in range(len(dynamic_thres_class))], dtype=torch.float32, device=device)
    return dynamic_thres_class, new_class_thresholds
...
def train_sfda(cfg, model_student, model_teacher, resume=False):
    ...
    # useless
    dynamic_thres_class = {i: torch.tensor(0, dtype=torch.float, device=model_teacher.device) for i in range(cfg.MODEL.ROI_HEADS.NUM_CLASSES)}
    with EventStorage(start_iter) as storage:
        for epoch in range(1, total_epochs+1):
            ...
            for data, iteration in zip(data_loader, range(start_iter, max_iter)):
                ...
                dynamic_thres_class, new_class_thresholds = update_class_scores(teacher_results[0].scores, teacher_results[0].pred_classes, dynamic_thres_class, thres=0.7)
                
                teacher_pseudo_results, num_roih_proposal = process_pseudo_label(teacher_results, new_class_thresholds, "roih", "thresholding")
